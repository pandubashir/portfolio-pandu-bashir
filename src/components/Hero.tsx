...
const SPLINE_SCENE_URL = 'https://prod.spline.design/Im6PHeUgHW7FmVsy/scene.splinecode'
...
export function Hero() {
  ...
  return (
    ...
    >
      {/* Robot Spline sebagai background penuh, posisi tengah section, di belakang teks & foto */}
      <div className="absolute inset-0 z-0 flex items-center justify-center">
        <div className="relative w-full h-full max-w-[1100px] mx-auto scale-90 opacity-75">
          <SplineScene scene={SPLINE_SCENE_URL} className="w-full h-full" />
          {/* Penutup watermark Spline — nempel persis di pojok kanan-bawah canvas robot */}
          <div
            className="absolute bottom-0 right-0 w-72 h-20 pointer-events-none"
            style={{ background: 'linear-gradient(to top left, #080b12 65%, transparent 100%)' }}
          />
          <div className="absolute bottom-0 right-0 w-56 h-12 bg-[#080b12] pointer-events-none rounded-tl-2xl" />
        </div>
      </div>

      {/* Overlay halus biar teks tetap kebaca di atas robot */}
      ...
      <div className="flex-1 relative z-10">
        ...
        <p className="text-sm text-muted2 mb-2 tracking-widest uppercase"
          style={{ fontFamily: "'DM Sans', sans-serif" }}>Hey, I'm Pandu 👋</p>
        <h1 className="text-[2.6rem] md:text-[4.8rem] font-bold leading-[1.02] tracking-[-2px] mb-4">
          <span className="glass-text">Software</span>
          <br />
          <span className="bg-gradient-to-br from-accent via-accent2 to-accent3 bg-clip-text text-transparent"
        ...
        <p className="text-[0.88rem] text-muted2/80 max-w-[420px] mb-10 leading-[1.9] mx-auto md:mx-0 font-light tracking-wide"
          style={{ fontFamily: "'DM Sans', sans-serif" }}>
          A Software Engineer passionate about Data Science, Data Analysis, and building functional
          systems you can rely on. Currently exploring machine learning, web development, and
          digital forensics.
        </p>
      ...
      {/* Foto profil — turun sedikit dari posisi sebelumnya */}
      <div className="relative z-10 flex-shrink-0 w-full md:w-[300px] h-[300px] md:h-[500px] md:-mr-10 md:mt-8">
        <img
          ref={imgRef}
          id="hero-img"
          className="hero-profile-img relative z-10 w-full h-full object-cover object-top block transition-transform duration-100 ease-out"
          src="/poto-pandu.jpeg"
